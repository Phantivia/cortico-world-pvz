...
The World runs this script itself: `native-build.ts` hashes every file in this directory,
builds into `<worlds.pvz.nativeBuildDir>/<hash>/` (default `<deployment root>/runtimes/pvz/`)
and writes `artifacts.json` with both binaries' SHA-256. A directory whose manifest or binaries
no longer match is renamed aside and rebuilt. Nothing is downloaded; the toolchain is the only
prerequisite. Adapting the bridge to another executable is described in
[ADAPTING.md](ADAPTING.md).

The pointer and companion sprite are not in this directory: `implant.cpp` includes
`cursor-companion/generated/cursor_companion.h` from the repository root, generated from the
JSON sources in [`cursor-companion/`](../../cursor-companion/README.md). That header is part of
the build identity.
