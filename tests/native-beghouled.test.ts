...
it.runIf(process.platform === 'win32')('native twist targets require a complete occupied square', async () => {
  ...
  const root = mkdtempSync(join(scratch, 'pvz-native-beghouled-'));
  const script = join(root, 'build-fixture.cmd');
  const source = fileURLToPath(new URL('./native-beghouled.cpp', import.meta.url));
