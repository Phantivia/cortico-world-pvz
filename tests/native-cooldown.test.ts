...
it.runIf(process.platform === 'win32')('native card observations count down as recharge advances', async () => {
  ...
  const root = mkdtempSync(join(scratch, 'pvz-native-cooldown-'));
  const script = join(root, 'build-fixture.cmd');
  const source = fileURLToPath(new URL('./native-cooldown.cpp', import.meta.url));
