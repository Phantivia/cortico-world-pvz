...
it.runIf(process.platform === 'win32')('native board identity permits elapsed frames and rejects run changes', async () => {
  ...
  const root = mkdtempSync(join(scratch, 'pvz-native-board-context-'));
  const script = join(root, 'build-fixture.cmd');
  const source = fileURLToPath(new URL('./native-board-context.cpp', import.meta.url));
