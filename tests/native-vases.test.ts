...
it.runIf(process.platform === 'win32')('native vase observation separates drawn markings from concealed contents', async () => {
  ...
  const root = mkdtempSync(join(scratch, 'pvz-native-vases-'));
  const script = join(root, 'build-fixture.cmd');
  const source = fileURLToPath(new URL('./native-vases.cpp', import.meta.url));
