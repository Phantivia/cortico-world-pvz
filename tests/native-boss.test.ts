...
it.runIf(process.platform === 'win32')('native boss observation preserves visible projectiles and mower conditions', async () => {
  ...
  const root = mkdtempSync(join(scratch, 'pvz-native-boss-'));
  const script = join(root, 'build-fixture.cmd');
  const source = fileURLToPath(new URL('./native-boss.cpp', import.meta.url));
