...
it.runIf(process.platform === 'win32')('native planting and user dialogs execute against memory fixtures', async () => {
  ...
  const root = mkdtempSync(join(scratch, 'pvz-native-relative-fixture-'));
  const script = join(root, 'build-fixture.cmd');
  const source = fileURLToPath(new URL('./native-relative-plant.cpp', import.meta.url));
  ...
  execFileSync(join(root, 'fixture.exe'), [],
