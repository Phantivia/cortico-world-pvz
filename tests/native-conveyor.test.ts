...
it.runIf(process.platform === 'win32')('native conveyor packets expose only visible clickable card regions', async () => {
  ...
  const root = mkdtempSync(join(scratch, 'pvz-native-conveyor-'));
  const script = join(root, 'build-fixture.cmd');
  const source = fileURLToPath(new URL('./native-conveyor.cpp', import.meta.url));
