...
it.runIf(process.platform === 'win32')('native observations retain right-edge portals and reject other off-board items', async () => {
  ...
  const root = mkdtempSync(join(scratch, 'pvz-native-portals-'));
  const script = join(root, 'build-fixture.cmd');
  const source = fileURLToPath(new URL('./native-portals.cpp', import.meta.url));
