...
it.runIf(process.platform === 'win32')('native mouse dispatch accepts background input and restores focus', async () => {
  ...
  const root = mkdtempSync(join(scratch, 'pvz-native-mouse-'));
  const script = join(root, 'build-fixture.cmd');
  const source = fileURLToPath(new URL('./native-mouse.cpp', import.meta.url));
