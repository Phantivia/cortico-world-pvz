...
it.runIf(process.platform === 'win32')('visible ice blocks planting until cleared and stays hidden under fog', async () => {
  ...
  const root = mkdtempSync(join(scratch, 'pvz-native-ice-'));
  const script = join(root, 'build-fixture.cmd');
  const source = fileURLToPath(new URL('./native-ice.cpp', import.meta.url));
