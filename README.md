...
- Plant descriptions include Tall-nut's interception of Pole Vaulting and Pogo jumps. Jack-in-the-Box observations distinguish carrying the box from opening it before the explosion. The environment prompt describes combining packet appearance with an empty chosen cell for planned defense placement.
- Planted backward shooters retain their firing direction in every board view. Vasebreaker packets may be deployed in any legal row; the environment prompt covers allocating them across remaining threats.
...
- Roll, gem move, zombie placement, trophy purchase, onslaught transition, garden action, and cannon shot end their skill queue. Vase breaking, Bowling, and Zombiquarium purchases/feedings may share a finite queue; each step checks current targets and resources and verifies its result before the next step.
