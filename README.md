...
`pvz_arm({when, steps, queue?, expiresInMs?, maxFirings?})` submits the chosen queue when its condition becomes true (`queue` defaults to `now`). It fires once by default; `maxFirings:1..16` permits a bounded number of submissions. After each submission the condition must be observed false before a later true observation can fire again; unknown observations do not reset this latch. A failed action still uses one firing. Triggers do not block the executor or bind packet slots, and the submitted queue's own `when` handles readiness. Conveyor admission counts all remaining firings against current card quantities. `expiresInMs` counts from arming and withdraws remaining firings. A trigger is bound to its board run; a terminal screen or another run withdraws it. Every outcome — fired, expired, cancelled, invalidated — is a `pvz.trigger` event, and each submitted queue reports through `pvz.task`. Trigger and task ids share one sequence.
